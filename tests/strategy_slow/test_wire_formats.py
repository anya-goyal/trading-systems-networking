...
def build_mdh_packet_full(
    ...
    header = struct.pack(ss.MDH_HDR_FMT, len(body))
    return header + body
...
def test_mdh_symbol_padding_and_decoding():
    ...
    msg = ss.parse_mdh_packet(packet)
...
def test_mdh_fixed_body_size():
    assert ss.MDH_BODY_SIZE == struct.calcsize(ss.MDH_BODY_FMT)
...
def test_iog_order_fixed_size():
    _, raw = ss.build_new_order("AAPL", config.SIDE_BUY, 10, 100.5)

    assert len(raw) == ss.IOG_ORDER_SIZE
...
def test_iog_length_prefix():
    ...
    expected_body_len = ss.IOG_ORDER_SIZE - 2
...
def test_iog_message_type():
    ...
    assert msg_type == ss.IOG_NEW_ORDER


def test_iog_client_order_id_padding():
    cl_id, raw = ss.build_new_order("AAPL", config.SIDE_BUY, 10, 100.5)
...
def test_iog_side_and_order_type():
    ...
    assert order_type == ss.ORD_LIMIT


def test_iog_quantity_and_price():
    _, raw = ss.build_new_order("AAPL", config.SIDE_BUY, 123, 99.75)
...
def test_iog_destination_and_strategy_id():
    ...
    assert dest == ss.DEST_EXCHANGE_1
    assert strategy_id == ss.STRATEGY_ID


def test_iog_symbol_truncation():
    long_symbol = "TOOLONGSYM"
    _, raw = ss.build_new_order(long_symbol, config.SIDE_BUY, 10, 100.5)
