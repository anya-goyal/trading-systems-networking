...
def main() -> None:
    ...
    # print("[DEBUG] created Rate Limiter")
    while True:
        readable, _, _ = select.select(mdh_sockets, [], [], 0.001)
        # print("[DEBUG] in true loop")

        for sock in readable:
            try:
                # print("[DEBUG] trying to read UDP message")
                data, _ = sock.recvfrom(config.UDP_BUF_SIZE)
            except BlockingIOError:
                # print("[DEBUG] BlockingIOError")
                continue
            except OSError as exc:
