...
Fast strategy.
  ...
  - Does NOT handle dropped packets — prioritizes speed over completeness
  - Minimal processing, simple reactive logic (to be implemented)
...
def main():
    ...
    print(f"[Fast Strategy] Subscribed to multicast group 1 ({config.MULTICAST_GROUP_1}:{config.MULTICAST_PORT_1})")
    ...
    print(f"[Fast Strategy] Subscribed to multicast group 2 ({config.MULTICAST_GROUP_2}:{config.MULTICAST_PORT_2})")
    ...
    print(f"[Fast Strategy] Connected to IOG at {config.IOG_HOST}:{config.IOG_PORT}")

    print("[Fast Strategy] Running.")
    while True:
        pass
